...
target_folder = "/opt/data/private/data/blueberry2_crop_loca"


image_source_folder = os.path.join(source_folder, "images_equalization_enhancement")
image_target_folder = os.path.join(target_folder, "images_equalization_enhancement")
...
for image_name in train_val_test_images:
    ...
    # 计算裁剪后子图的数量
    num_cols = new_w // crop_size[1]
    num_rows = new_h // crop_size[0]
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            box = (j * crop_size[1], i * crop_size[0], (j + 1) * crop_size[1], (i + 1) * crop_size[0])
            ...
            # 在points中添加示例框坐标
            # 裁剪points   
            cropped_points = torch.tensor(points) - torch.as_tensor([[j * crop_size[1], i * crop_size[0]]])
            ...
            annotation = {}
            annotation['H'] = crop_size[0]
            annotation['W'] = crop_size[1]
