...
class CNN(nn.Module):
    ...
    def forward(self, x):
        # 输入大小：[1, 3, 4096, 4096]
        x = F.relu(self.conv1(x))  # 输出大小：[1, 64, 2048, 2048]
        x = F.relu(self.conv2(x))  # 输出大小：[1, 128, 1024, 1024]
        x = F.relu(self.conv3(x))  # 输出大小：[1, 256, 512, 512]
        x = F.relu(self.conv4(x))  # 输出大小：[1, 128, 512, 512]
        x = F.relu(self.conv5(x))  # 输出大小：[1, 64, 512, 512]
        x = F.relu(self.conv6(x))  # 输出大小：[1, 3, 512, 512]
...
class FSC147Dataset(Dataset):
    ...
    def __init__(
        ...
        with open(
            os.path.join(self.data_path, 'train_val_test.json'), 'rb'
        ) as file:
            splits = json.load(file)
            self.image_names = splits[split]
        with open(
            os.path.join(self.data_path, 'annotation.json'), 'rb'
        ) as file:
    ...
    def __getitem__(self, idx: int):
        ...
        # 换成384
        resize_density_map = T.Resize((512, 512))

        original_sum = density_map.sum()
        density_map = resize_density_map(density_map)
        density_map = density_map / density_map.sum() * original_sum
        # data augmentation
        tiled = False
        if self.split == 'train' and torch.rand(1) < self.tiling_p:
            ...
            img, bboxes, density_map = tiling_augmentation(
                img, bboxes, density_map, resize_density_map,
        ...
        return img, bboxes, density_map, imgname

    def __len__(self):
        return len(self.image_names)


def generate_density_maps(data_path, target_size=(4096, 4096)):

    density_map_path = os.path.join(
        data_path,
        f'gt_density_map_adaptive_{target_size[0]}_{target_size[1]}_blueberry'
    )
    if not os.path.isdir(density_map_path):
        os.makedirs(density_map_path)

    with open(
        os.path.join(data_path, 'annotation.json'), 'rb'
    ) as file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', default=4096, type=int)
