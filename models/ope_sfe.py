...
class OPEModule(nn.Module):
    ...
    def __init__(
        ...
        if not self.zero_shot:
            ...
            # )
            self.shape_or_objectness = nn.ModuleList([
                # loca-sfe
                ...
                # loca-sfe2
                nn.Conv2d(3, 32, kernel_size=2, padding=1, stride=2),
                # relu在外面
                nn.Conv2d(32, 3, kernel_size=2, padding=1, stride=2),
                # relu在外面
                nn.Linear(4, 64),
                nn.ReLU(),
                nn.Linear(64, emb_dim),
                ...
                # loca-sfe4
                # nn.Conv2d(3, 64, kernel_size=2, padding=1, stride=2),
                # # relu在外面
                # nn.Conv2d(64, 3, kernel_size=2, padding=1, stride=2),
                # # relu在外面
                # nn.Linear(4, 64),
                # nn.ReLU(),
                # nn.Linear(64, emb_dim),
                ...
                # loca-sfe5
                # nn.Conv2d(3, 128, kernel_size=2, padding=1, stride=2),
                # # relu在外面
                # nn.Conv2d(128, 3, kernel_size=2, padding=1, stride=2),
                # # relu在外面
                # nn.Linear(4, 64),
                # nn.ReLU(),
                # nn.Linear(64, emb_dim),
                ...
                # nn.Conv2d(16, 3, kernel_size=2, padding=1, stride=2),
                # nn.Linear(4, 64),
                # nn.ReLU(),
                # nn.Linear(64, emb_dim),
                # nn.ReLU(),
    ...
    def forward(self, f_e, pos_emb, bboxes):
        ...
        if not self.zero_shot:
            ...
            # box_hw 是一个形状为 [batch_size, num_boxes, 2] 的张量
            # 首先将其转换为一个形状为 [batch_size, num_boxes, 1, width] 的张量
            box_hw = box_hw.unsqueeze(2)  # 添加一个额外的维度
            box_hw = box_hw.repeat(1, 1, 2, 1)  # 重复扩展以匹配卷积层的输入通道数
        
            x = box_hw
            for layer in self.shape_or_objectness:
                if not isinstance(layer, nn.Conv2d):
                    # 形状
                    x = x.view(x.size(0), x.size(1), -1)  # 展平特征图
                    x = layer(x)
                else:
                    x = F.relu(layer(x))
            shape_or_objectness = x.reshape(
