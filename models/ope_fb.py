...
class OPEModule(nn.Module):
    ...
    def __init__(
        ...
        if not self.zero_shot:
            ...
            self.feedback_adjuster = nn.Sequential(
                # 输入数据的形状为 (batch_size, num_boxes, 2)，输出形状为 (num_boxes, 1)
                nn.Linear(3, 64),
                nn.ReLU(),
                nn.Linear(64, emb_dim),
                nn.ReLU(),
    ...
    def forward(self, f_e, pos_emb, bboxes, feedback=None):
        bs, _, h, w = f_e.size()
        ...
        if not self.zero_shot:
            ...
            if feedback is not None:
                ...
                shape_features = self.shape_or_objectness(box_hw)
                adjusted_features = torch.cat([shape_features, feedback], dim=1)
                adjusted_features = self.feedback_adjuster(adjusted_features)
                shape_or_objectness = adjusted_features.reshape(
...
class IterativeAdaptationModule(nn.Module):
    ...
    def forward(
        ...
        # 迭代的结果作为feedback_data反馈到形状特征提取模块？
